...
class BatchEpisodes(object):
    def __init__(self, batch_size, gamma=0.95, device='cpu'):
        ...
        self._next_obs_list = [[] for _ in range(batch_size)]
        self._mask_list = []
    ...
    def next_obs(self):
        if self._next_obs is None:
            obs_shape = self._next_obs_list[0][0].shape
            next_obs = np.zeros((len(self), self.batch_size) + obs_shape, 
                    dtype=np.float32)
            for i in range(self.batch_size):
                length = len(self._next_obs_list[i])
                next_obs[:length, i] = np.stack(self._next_obs_list[i], axis=0)
            self._next_obs = torch.from_numpy(next_obs).to(self.device)
    ...
    def rewards(self):
        if self._rewards is None:
            ...
            for i in range(self.batch_size):
                ...
                rewards[:length, i] = np.stack(self._rewards_list[i], axis=0).reshape(-1)
    ...
    def append(self, observations, actions, rewards, next_obs, batch_ids):
        for observation, action, reward, next_ob, batch_id in zip(
                observations, actions, rewards, next_obs, batch_ids):
            if batch_id is None:
                continue
            ...
            self._next_obs_list[batch_id].append(next_ob.astype(np.float32))

    def append_traj(self, s_traj, a_traj, r_traj, ns_traj, id_traj):
        self.s_traj = s_traj
        self.a_traj = a_traj
        self.r_traj = r_traj
        self.ns_traj = ns_traj
        self.id_traj = id_traj

"""
SUMO Environment Wrapper for PPO Training
Implements a gym-compatible interface for SUMO traffic control
Tối ưu cho PPO:
- Mỗi env.step() chỉ thực hiện 1 SUMO simulation step
- Trả về reward per step (không tích lũy) để tính GAE chính xác
- Phase duration được quản lý bên trong: chỉ reset khi phase thay đổi hoặc hết thời gian
"""
...
class SUMOEnv(gym.Env):
    ...
    def __init__(self, sumo_config_path, max_steps=3600, yellow_time=3, green_time_min=10, green_time_max=60, use_gui=False):
        ...
        # Phase duration tracking for PPO (tối ưu cho PPO)
        self._current_phase_duration = None
        self._phase_duration_remaining = 0
        self._last_phase_in_sumo = None
    ...
    def _get_reward(self):
        ...
        if lane_count > 0:
            ...
            # ===== OPTIMIZED REWARD SHAPING FOR PPO =====
            # PPO works best with stable, well-scaled rewards
            # Focus on key metrics: queue length (penalty) and speed (reward)
            
            # 1. Primary penalty: Queue length (main congestion indicator)
            # Normalize to reasonable scale for PPO stability
            reward -= avg_queue * 0.5
            
            # 2. Secondary penalty: Waiting time (accumulated delay)
            reward -= avg_waiting * 0.05
            
            # 3. Primary reward: Speed (traffic flow efficiency)
            if avg_speed > 0:
                # Normalize speed to [0, 1] range (assuming max ~20 m/s)
                speed_norm = min(avg_speed / 20.0, 1.0)
                reward += speed_norm * 2.0
            
            # 4. Throughput bonus: Reward for vehicles passing through
            reward += avg_vehicles * 0.1
            
            # 5. Smoothing bonus: Reward for improving conditions
            # This helps PPO learn gradual improvements
            if hasattr(self, '_prev_queue') and self._prev_queue > 0:
                queue_improvement = (self._prev_queue - avg_queue) / (self._prev_queue + 1e-6)
                if queue_improvement > 0:
                    reward += queue_improvement * 1.0
            self._prev_queue = avg_queue
            
            if hasattr(self, '_prev_speed') and self._prev_speed > 0:
                speed_improvement = (avg_speed - self._prev_speed) / (self._prev_speed + 1e-6)
                if speed_improvement > 0:
                    reward += speed_improvement * 0.5
            self._prev_speed = avg_speed
        
        # Clip reward to reasonable range for PPO stability
        # Typical range: -50 to +50 for most steps
    ...
    def step(self, action):
        ...
        # Tối ưu cho PPO: Chỉ reset phase duration khi phase thực sự thay đổi trong SUMO
        # Hoặc khi phase duration hết thời gian
        if (self._last_phase_in_sumo is None or 
            current_phase != self._last_phase_in_sumo or 
            self._phase_duration_remaining <= 0):
            # Phase đã thay đổi hoặc duration hết → áp dụng phase duration mới
            self._current_phase_duration = phase_duration
            self._phase_duration_remaining = phase_duration
            self._last_phase_in_sumo = current_phase
        
        # TỐI ƯU CHO PPO: Chỉ thực hiện 1 SUMO simulation step mỗi lần env.step()
        # Điều này đảm bảo PPO nhận được reward per step để tính GAE chính xác
        # QUAN TRỌNG: Luôn thực hiện SUMO step nếu chưa đạt max_steps
        # (không phụ thuộc vào phase_duration_remaining để đảm bảo PPO luôn nhận được step hợp lệ)
        reward = 0.0
        
        # Execute one SUMO simulation step (tối ưu cho PPO)
        if self.step_count < self.max_steps:
            try:
                traci.simulationStep()
                self.step_count += 1
                
                # Decrement phase duration remaining (nếu > 0)
                if self._phase_duration_remaining > 0:
                    self._phase_duration_remaining -= 1
                
                # Get reward for this single step (critical for PPO GAE calculation)
                try:
                    reward = self._get_reward()
                except (traci.exceptions.FatalTraCIError, traci.exceptions.TraCIException, Exception) as e:
                    # Connection lost during reward collection
                    ...
                    try:
                        ...
                        return observation, 0.0, True, False, {'error': str(e2), 'step_count': self.step_count}
            except (traci.exceptions.FatalTraCIError, traci.exceptions.TraCIException, Exception) as e:
        ...
        info = {
            ...
            'phase': current_phase,
            'phase_duration': self._current_phase_duration if self._current_phase_duration else phase_duration,
            'phase_duration_remaining': self._phase_duration_remaining,
        ...
        # IMPORTANT: Return reward per step (not accumulated) for PPO compatibility
        terminated = done

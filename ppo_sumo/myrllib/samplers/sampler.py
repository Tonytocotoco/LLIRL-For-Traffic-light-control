...
def make_env(env_name, sumo_config_path=None, seed=0):
    def _make_env():
...
class BatchSampler(object):
    def __init__(self, env_name, batch_size, num_workers=1, seed=0, sumo_config_path=None):
        self.env_name = env_name
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.sumo_config_path = sumo_config_path
        
        self._env = gym.make(env_name, sumo_config_path=sumo_config_path)
        ...
        if num_workers == 0:
            ...
            self.queue = mp.Queue()
            self.envs = SubprocVecEnv([make_env(env_name, sumo_config_path, seed=seed) for _ in range(num_workers)],
                queue=self.queue)
